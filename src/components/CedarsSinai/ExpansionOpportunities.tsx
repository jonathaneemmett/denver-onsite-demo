...
export function ExpansionOpportunities({ selectedYear }: ExpansionOpportunitiesProps) {
	...
	const getOpportunityColor = (opportunity: string) => {
		switch (opportunity) {
			case 'high': return 'bg-red-50 border border-red-100 text-red-700';
			case 'medium': return 'bg-amber-50 border border-amber-100 text-amber-700';
			case 'low': return 'bg-emerald-50 border border-emerald-100 text-emerald-700';
	...
	const getStructureColor = (structure: string) => {
		if (structure.includes('Cedars-Sinai')) return 'from-slate-600 to-slate-700';
		if (structure.includes('UCLA')) return 'from-slate-500 to-slate-600';
		if (structure.includes('Providence')) return 'from-slate-600 to-slate-700';
						...
						<div className={`bg-gradient-to-r ${getStructureColor(structure)} px-4 py-3`}>
							<div className="flex items-center justify-between">
								<div className="flex items-center">
									<FontAwesomeIcon icon={faBuilding} className="w-4 h-4 text-white mr-3" />
									<h3 className="text-base font-medium text-white">{structure}</h3>
								</div>
								<div className="text-slate-200 text-xs font-medium">
									{hospitals.length} {hospitals.length === 1 ? 'location' : 'locations'}
								</div>
							</div>
						</div>
									...
									>
										<div className="flex items-center justify-between">
											<div className="flex items-center space-x-4 flex-1">
												<FontAwesomeIcon 
													icon={faHospital} 
													className={`w-4 h-4 ${hospital.id === 'csmc' ? 'text-slate-700' : 'text-slate-500'} group-hover:text-slate-700 transition-colors`} 
												/>
												<div className="flex-1">
													<div className="flex items-center gap-2">
														...
														<button
															onClick={(e) => openHospitalProfile(hospital, e)}
													...
													<div className="flex items-center text-sm text-slate-500 mt-1">
														...
														{hospital.distance > 0 && (
															<span className="ml-2 text-slate-400">• {hospital.distance} mi</span>
														)}
													</div>
												</div>
											</div>
											...
											<div className="flex items-center space-x-6">
												...
												<div className="text-center">
													<div className="flex items-center text-xs text-slate-500 justify-center">
														<FontAwesomeIcon icon={faHeart} className="w-3 h-3 text-red-400 mr-1" />
														<span>CMR</span>
													</div>
													<p className="text-sm font-semibold text-slate-800">{hospital.cmrVolume}</p>
												</div>

												{/* Chest MRI Volume */}
												<div className="text-center">
													<div className="flex items-center text-xs text-slate-500 justify-center">
														<FontAwesomeIcon icon={faLungs} className="w-3 h-3 text-blue-400 mr-1" />
														<span>Chest</span>
													</div>
													<p className="text-sm font-semibold text-slate-800">{hospital.chestMriVolume}</p>
												</div>

												{/* Market Share */}
												<div className="text-center">
													<p className="text-xs text-slate-500">Share</p>
													<p className="text-sm font-semibold text-slate-800">{hospital.marketShare}%</p>
												</div>
												...
												<div className="text-center">
													<div className="flex items-center justify-center">
														<FontAwesomeIcon
															icon={getTrendIcon(hospital.growthTrend, hospital.growthPercent).icon}
															className={`w-3 h-3 mr-1 ${getTrendIcon(hospital.growthTrend, hospital.growthPercent).color}`}
														/>
														<span className={`text-xs font-medium ${getTrendIcon(hospital.growthTrend, hospital.growthPercent).color}`}>
															{Math.abs(hospital.growthPercent)}%
														</span>
													</div>
												</div>

												{/* Opportunity Level */}
												<div className={`px-2 py-1 rounded-md text-xs font-medium ${getOpportunityColor(hospital.opportunity)}`}>
												...
												<FontAwesomeIcon
													...
													className={`w-3 h-3 text-slate-400 transition-transform duration-200 ${
														selectedHospital === hospital.id ? 'rotate-90' : ''
													}`}
												/>
											</div>
										</div>
										...
										<div className="bg-slate-50 border-t border-slate-100 p-6">
											<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
												...
												<div className="bg-white rounded-lg p-4 border border-slate-200">
													<h5 className="font-medium text-slate-900 mb-3 text-sm">Volume Breakdown</h5>
													<div className="space-y-2">
														<div className="flex justify-between items-center">
															<span className="text-xs text-slate-600">CMR Studies:</span>
															<span className="font-medium text-slate-800 text-sm">{hospital.cmrVolume}</span>
														</div>
														<div className="flex justify-between items-center">
															<span className="text-xs text-slate-600">Chest MRI:</span>
															<span className="font-medium text-slate-800 text-sm">{hospital.chestMriVolume}</span>
														</div>
														<div className="flex justify-between items-center pt-2 border-t border-slate-100">
															<span className="text-xs font-medium text-slate-900">Total:</span>
															<span className="font-semibold text-slate-900 text-sm">{hospital.totalVolume}</span>
														</div>
													</div>
												</div>
												...
												<div className="bg-white rounded-lg p-4 border border-slate-200">
													<h5 className="font-medium text-slate-900 mb-3 text-sm">Market Analysis</h5>
													<div className="space-y-2">
														<div className="flex items-center justify-between">
															...
															<div className="flex items-center">
																...
																/>
																<span className={`text-xs font-medium ${getTrendIcon(hospital.growthTrend, hospital.growthPercent).color}`}>
																	{hospital.growthPercent > 0 ? '+' : ''}{hospital.growthPercent}%
																</span>
															</div>
														</div>
														<div className="flex justify-between items-center">
															<span className="text-xs text-slate-600">Market Share:</span>
															<span className="font-medium text-slate-800 text-sm">{hospital.marketShare}%</span>
														</div>
														<div className={`px-2 py-1 rounded text-center mt-2 ${getOpportunityColor(hospital.opportunity)}`}>
															<span className="text-xs font-medium">{hospital.opportunity} opportunity</span>
														</div>
													</div>
												</div>
												...
												<div className="bg-white rounded-lg p-4 border border-slate-200 flex flex-col justify-center">
													<button
														onClick={(e) => openHospitalProfile(hospital, e)}
														className="bg-slate-700 hover:bg-slate-800 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors flex items-center justify-center gap-2"
													>
														<FontAwesomeIcon icon={faChevronRight} className="w-3 h-3" />
														View Full Profile
													</button>
