...
export function StructuralHeartCapture({
	selectedYear,
	hospitalName = 'Cedars-Sinai',
}: StructuralHeartCaptureProps) {
	const [selectedView, setSelectedView] = useState<
		'overview' | 'geographic' | 'billing'
	>('overview');

	// Get data for selected year
	const mockRegionalData =
		mockRegionalDataByYear[selectedYear] || mockRegionalDataByYear[2025];
	const mockMyocarditisData =
		mockMyocarditisDataByYear[selectedYear] ||
		mockMyocarditisDataByYear[2025];

	const totalRegionalMyocarditis = mockRegionalData.reduce(
		(sum, hospital) => sum + hospital.myocarditisVolume,
		0,
	);
	const totalRegionalStructural = mockRegionalData.reduce(
		(sum, hospital) => sum + hospital.structuralProcedures,
		0,
	);
	const overallConversionRate =
		(totalRegionalStructural / totalRegionalMyocarditis) * 100;
	const cedarsSinaiData = mockRegionalData[0];
	const competitorAvgConversion =
		mockRegionalData
			.slice(1)
			.reduce((sum, h) => sum + h.conversionRate, 0) /
		(mockRegionalData.length - 1);
	...
	const getSubtitle = () => {
		const baseSubtitle =
			'Analyze myocarditis patient volumes and structural heart procedure conversion rates within 50-mile radius';
		if (isPredictiveYear) {
			...
			<div className='flex space-x-1 mb-6 bg-neutral-100 rounded-lg p-1'>
				{[
					...
					<button
						...
						onClick={() =>
							setSelectedView(
								key as 'overview' | 'geographic' | 'billing',
							)
						}
					...
					<div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
						<div className='bg-blue-50 rounded-lg p-4'>
							...
							<p className='text-2xl font-bold text-blue-800'>
								{totalRegionalMyocarditis}
							</p>
							<p className='text-sm text-blue-700'>
								Annual Myocarditis Cases
							</p>
						</div>
						...
						<div className='bg-amber-50 rounded-lg p-4'>
							...
							<p className='text-2xl font-bold text-amber-800'>
								$
								{(
									mockMyocarditisData.revenueOpportunity /
									1000000
								).toFixed(1)}
								M
							</p>
						...
						<div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
							<div className='text-center'>
								<p className='text-3xl font-bold text-blue-700 mb-1'>
									{cedarsSinaiData.myocarditisVolume}
								</p>
							...
							<div className='text-center'>
								...
								<p className='text-xs text-purple-600 bg-purple-100 px-2 py-1 rounded-full'>
									vs {competitorAvgConversion.toFixed(1)}%
									competitor avg
								</p>
							</div>
						</div>
					...
					<div className='bg-neutral-50 rounded-lg p-4 mb-6'>
						<div className='flex items-center mb-2'>
							...
							<h3 className='text-lg font-semibold text-neutral-900'>
								Regional Hospital Analysis
							</h3>
						</div>
						<p className='text-sm text-neutral-600'>
							Myocarditis diagnosis to structural heart procedure
							conversion within 50-mile radius of Cedars-Sinai
						</p>
					</div>
										...
										<td className='border border-neutral-300 px-4 py-4 text-center'>
											<span
												className={`text-lg font-bold px-3 py-1 rounded-full ${
													index === 0 &&
													hospital.conversionRate >
														competitorAvgConversion
												...
												{hospital.conversionRate.toFixed(
													1,
												)}
												%
											</span>
						...
						<div className='flex items-start'>
							...
							<div>
								<h4
									...
									{isPredictiveYear
										? 'Statistical Projections with Software Implementation'
										: 'Key Insights'}
								</h4>
								<ul
									...
									{isPredictiveYear ? (
										<>
											<li>
												• Cedars-Sinai conversion rate
												projected to reach{' '}
												{cedarsSinaiData.conversionRate.toFixed(
													1,
												)}
											...
											<li>
												...
												increase to{' '}
												{cedarsSinaiData.marketShare.toFixed(
													1,
												)}
											...
											<li>
												...
												{(
													cedarsSinaiData.conversionRate -
													competitorAvgConversion
												).toFixed(1)}
										...
										<>
											<li>
												...
												overallConversionRate
													...
													: 'above'}{' '}
												regional average (
												{overallConversionRate.toFixed(
													1,
												)}
												%)
											</li>
											<li>
												• UCLA Medical Center maintains
												highest conversion rate at{' '}
												{mockRegionalData
													.find((h) =>
														h.name.includes('UCLA'),
													)
											...
											<li>
												...
												% market share with{' '}
												{totalRegionalMyocarditis} total
												cases
											</li>
											<li>
												• Revenue opportunity: $
				...
				<div className='space-y-6'>
					<div className='bg-gradient-to-r from-slate-50 to-stone-50 rounded-lg p-4 mb-6 border border-slate-200'>
						...
						<p className='text-sm text-slate-600'>
							Estimated myocarditis patient volume based on
							current billing data and conversion opportunities
						</p>
					</div>
								...
								<div className='flex-1 mx-4'>
									<div className='flex items-center'>
										<div className='flex-1 h-3 bg-slate-200 rounded-full shadow-inner'>
											<div
												className='h-3 bg-gradient-to-r from-slate-500 to-slate-600 rounded-full shadow-sm'
												style={{ width: '100%' }}></div>
										</div>
										<FontAwesomeIcon
											icon={faArrowRight}
								...
								<div className='flex-1 mx-4'>
									<div className='flex items-center'>
										<div className='flex-1 h-3 bg-amber-200 rounded-full shadow-inner'>
											<div
												className='h-3 bg-gradient-to-r from-amber-500 to-amber-600 rounded-full shadow-sm'
												style={{
													width: `${
														(mockMyocarditisData.currentCapture /
															mockMyocarditisData.potentialCapture) *
